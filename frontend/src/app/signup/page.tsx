...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We call our backend to verify if the user exists in the database
      const checkRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/check`, {
          method: "POST",
      ...
      // 3. Supabase Signup (Only proceeds if backend said "False")
    ...
    <div className="min-h-screen bg-[#0B0E14] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#151921] border border-white/10 p-8 rounded-2xl shadow-2xl relative z-10">
        ...
        <div className="text-center mb-8">
            <div className="w-16 h-16 bg-indigo-500/10 rounded-2xl flex items-center justify-center mx-auto mb-6 rotate-3 hover:rotate-6 transition-transform">
                <UserPlusIcon className="h-8 w-8 text-indigo-500" />
            </div>
            <h1 className="text-3xl font-bold text-white tracking-tight mb-2">Create Account</h1>
            <p className="text-slate-400 text-sm">Start tracking institutional market data today.</p>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 text-center">
          <p className="text-slate-500 text-sm">
            ...
            <Link href="/login" className="text-indigo-400 hover:text-indigo-300 font-bold hover:underline transition-colors">
              Log In
            </Link>
          </p>
        </div>
