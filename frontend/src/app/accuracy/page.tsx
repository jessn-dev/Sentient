...
export default function AccuracyPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/watchlist/performance`, {
          headers: {
            "Authorization": `Bearer ${session.access_token}` // <--- SECURE HEADER
          }
        });
  ...
  // Fetch Graph Data (Public Endpoint - No Auth Needed)
  useEffect(() => {
    if (selectedItem) {
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-white p-8 relative">
      ...
      <div className="max-w-6xl mx-auto bg-slate-900/50 rounded-2xl border border-slate-800 overflow-hidden">
        {loading ? <div className="p-12 text-center text-slate-500 animate-pulse">Loading your portfolio...</div> : (
          items.length === 0 ? (
            <div className="p-12 text-center text-slate-500">
              No active predictions found. <Link href="/" className="text-blue-400 hover:underline">Track a stock</Link> to get started.
            </div>
          ) : (
